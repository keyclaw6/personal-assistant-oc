...
sits on top of the file-based `memory/` directory, indexing files into a
...
```
memory/*.md  ──▶  Cognee API server (localhost:8000)  ──▶  Kuzu + LanceDB + SQLite
                       ▲
                       │
...
Both are gitignored. Regenerable from `memory/` source files.

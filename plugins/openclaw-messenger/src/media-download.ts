...
export function messengerMediaExtension(mediaType: string | undefined, url: string): string {
...
async function responseBytes(response: Response, maxBytes: number): Promise<Uint8Array> {
  ...
  if (contentLength > maxBytes) throw new Error(`media exceeds max bytes (${contentLength} > ${maxBytes})`);
  ...
  if (!reader) {
    ...
    if (bytes.byteLength > maxBytes) throw new Error(`media exceeds max bytes (${bytes.byteLength} > ${maxBytes})`);
    return bytes;
  }
  ...
  while (true) {
    ...
    if (total > maxBytes) throw new Error(`media exceeds max bytes (${total} > ${maxBytes})`);
    chunks.push(value);
  }
...
export async function downloadMessengerMedia(mediaUrls: string[], mediaTypes: string[] = [], opts: DownloadOptions = {}): Promise<MessengerMediaDownloadResult> {
